...
@section('title', 'Draft Form Commissioning')

@section('content')
    <x-user.page-header title="Draft Form Commissioning" subtitle="Lanjutkan form commissioning yang belum selesai." eyebrow="Draft Workspace">
        <a href="{{ route('user.commissioning.forms.create') }}" class="btn btn-primary">
    ...
    <section class="inspector-panel commissioning-list-panel">
        <div class="commissioning-list-toolbar">
            <div>
                <h2>Draft Aktif</h2>
                <p>{{ $submissions->total() }} draft menunggu dilanjutkan.</p>
            </div>
            <span class="commissioning-list-badge is-draft"><i class="bi bi-journal-richtext"></i>Draft</span>
        </div>
        <form method="GET" action="{{ route('user.commissioning.drafts.index') }}" class="commissioning-filter-bar">
            <label for="commissioning-draft-area">Area</label>
            <select id="commissioning-draft-area" name="area" class="form-select form-select-sm" onchange="this.form.submit()">
            ...
            @if (($selectedArea ?? 'all') !== 'all')
                <a href="{{ route('user.commissioning.drafts.index') }}" class="btn btn-sm btn-light commissioning-icon-btn" title="Reset filter" aria-label="Reset filter">
        ...
        <div class="table-responsive">
            <table class="table align-middle commissioning-list-table">
                <thead><tr><th>Form</th><th>Template</th><th>Equipment</th><th>Lokasi</th><th>Update</th><th class="text-end">Aksi</th></tr></thead>
                <tbody>
                    @forelse ($submissions as $submission)
                        <tr>
                            <td>
                                <div class="commissioning-form-no">{{ $submission->form_number }}</div>
                                <span class="commissioning-status-pill is-draft">Draft</span>
                            </td>
                            <td>
                            ...
                            <td>
                                ...
                                <small>{{ $submission->equipment_no ?: 'Belum dipilih' }}</small>
                            </td>
                            <td>
                                <div class="commissioning-main-text">{{ $submission->area ?: '-' }}</div>
                                <small>{{ $submission->functional_location ?: 'Belum ada functional location' }}</small>
                            </td>
                            <td>
                            ...
                            <td class="text-end">
                                <div class="commissioning-actions">
                                    <a href="{{ route('user.commissioning.submissions.edit', $submission) }}" class="btn btn-sm btn-primary commissioning-icon-btn" title="Lanjutkan" aria-label="Lanjutkan">
                    ...
                    @empty
                        <tr><td colspan="6" class="text-center text-muted py-5">Belum ada draft commissioning.</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
