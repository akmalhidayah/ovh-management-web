...
@section('title', 'Template Form Commissioning')
...
@section('content')
    ...
    <div class="page-header">
        <div>
            <h1>Template Form Commissioning</h1>
            <p>Kelola template form commissioning yang akan digunakan user Commissioning.</p>
        </div>
        <div class="page-actions">
            <a href="{{ route('admin.template-form-commissioning.create') }}" class="btn btn-primary">
        ...
        <div class="template-toolbar">
            <div class="template-tabs">
                @foreach ($tabs as $value => $label)
                    <a href="{{ route('admin.template-form-commissioning.index', array_filter(['status' => $value === 'all' ? null : $value, 'search' => request('search')])) }}"
            ...
            <table class="table align-middle template-table">
                ...
                <tbody>
                    @forelse ($templates as $template)
                        @php
                            $templateSchema = \App\Support\Commissioning\FixedCommissioningTemplate::normalizeSchema($template->body_schema);
                            $rowsCount = count($templateSchema['equipment_check_rows'] ?? []);
                        @endphp
                        <tr>
                            <td>
                            ...
                            <td>
                                <div class="template-actions">
                                    <a href="{{ route('admin.template-form-commissioning.preview', $template) }}" class="btn btn-sm btn-outline-primary">Preview</a>
                                    <a href="{{ route('admin.template-form-commissioning.edit', $template) }}" class="btn btn-sm {{ $template->status === 'draft' ? 'btn-primary' : 'btn-outline-secondary' }}">
                                    ...
                                    <div class="dropdown">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end">
                                            <li><a class="dropdown-item" href="{{ route('admin.template-form-commissioning.show', $template) }}">Detail</a></li>
                                            <li>
                                            ...
                                            @if ($template->status === 'active')
                                                <li>
                                                    <form action="{{ route('admin.template-form-commissioning.toggle-status', $template) }}" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                            ...
                                            @else
                                                <li>
                                                    <form action="{{ route('admin.template-form-commissioning.publish', $template) }}" method="POST">
                                                        @csrf
                                                        @method('PATCH')
                                            ...
                                            <li>
                                                <form action="{{ route('admin.template-form-commissioning.destroy', $template) }}" method="POST" onsubmit="return confirm('Hapus template ini?')">
                                                    @csrf
                                                    @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="text-center text-muted py-4">Belum ada template Form Commissioning.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
